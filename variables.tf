...
variable "fortigate_hostname" {
  description = <<-EOT
    FortiGate hostname to use for management connection.

    Options:
    - "management_ip": Connect via management interface IP (port1)
    - "public_ip": Connect via WAN interface IP (port2) - requires public IP
    - Custom IP or hostname

    When create_management_public_ip = true:
    - Use "management_ip" to connect via management public IP

    When create_management_public_ip = false:
    - Provide a reachable IP/hostname (e.g., via VPN, private network, bastion)

    The module will automatically resolve "management_ip" and "public_ip" to actual IPs.
  EOT
  type        = string
  default     = "management_ip"
}

variable "fortigate_connection_timeout" {
  description = <<-EOT
    FortiOS API connection timeout in seconds.

    Recommended values:
    - 300 (5 minutes) - Default, suitable for most deployments
    - 600 (10 minutes) - For slow network connections or high latency
    - 120 (2 minutes) - For fast networks with quick VM boot times

    The FortiGate VM typically takes 3-5 minutes to boot and be ready for API connections.
    Longer timeouts are safer for production deployments to account for:
    - VM boot time
  ...
  validation {
    condition     = var.fortigate_connection_timeout >= 60 && var.fortigate_connection_timeout <= 3600
    error_message = "Connection timeout must be between 60 (1 minute) and 3600 (1 hour) seconds."
  }
}

variable "fortigate_insecure_connection" {
  description = <<-EOT
    Skip SSL certificate verification for FortiOS API connection.

    SECURITY WARNING: Only set to true for:
    ...
    Options:
    - false: Verify SSL certificates (secure, production default)
    - true: Skip SSL verification (insecure, development only)

    Production recommendation: false (verify certificates)
    Development: Can be set to true for convenience with self-signed certs
  EOT
  type        = bool
  default     = true

  validation {
    condition     = var.fortigate_insecure_connection == true || var.environment != "dev"
    error_message = "For security, SSL verification should be enabled in non-dev environments. Set fortigate_insecure_connection = false for production."
  }
}

variable "fortigate_api_timeout" {
  description = <<-EOT
    FortiOS API request timeout in seconds for individual API operations.

    This is different from connection_timeout:
    - connection_timeout: How long to wait for initial connection to FortiGate
    - api_timeout: How long to wait for each API request to complete

    Recommended values:
    - 60 seconds: Default, suitable for most operations
    - 120 seconds: For large configuration changes or slow appliances
    - 30 seconds: For simple operations on fast appliances
  EOT
  type        = number
  default     = 60

  validation {
    condition     = var.fortigate_api_timeout >= 10 && var.fortigate_api_timeout <= 300
    error_message = "API timeout must be between 10 and 300 seconds."
  }
}
